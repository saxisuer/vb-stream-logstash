...
| `PipePathBenchmark` | **管道两口径**：`replayBucket` 预构造 2000 单元冻结桶（语料回卷 100 轮、捕获流式块内单元 ≈32.8MB，经 `BenchPipeBridge` 走组装器 reader 侧同构 append 记账落盘）逐段 readRange + decodeSingle + 快照 asOf 渲染——即 `TransactionConsumer.processBucket` 的回放半程；`appendOneMessage` 裸消息追加吞吐（无帧化，一条 CQ 记录即一条完整消息） | ms/桶；append 为 thrpt ops/s |

口径间关系：Decode 是 RoutePeek 两口径的对照上限；AssembleMemory 与 PipePathBenchmark.replayBucket 对照可见回放半程在组装总成本中的占比；AssembleMemory 与 Decode 对照可见组装开销中解码的占比。
