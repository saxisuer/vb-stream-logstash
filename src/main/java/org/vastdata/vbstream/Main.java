...
/** 冒烟入口：连上复制流，把 pgoutput 原始字节经 raw 驱动的 TransactionAssembler（纯 CQ index 段记账，数据经 MessagePipe 管道 append→readRange 往返）组装为原子事务块打印到控制台，Ctrl+C 优雅退出。 */
public final class Main {
     ...
     * 关键步骤：校验配置（含 pipe 配置解析，非法值启动期 fail-fast）→ 会话 open/ensureSlot/start →
     * reader 线程内 try-with-resources 建组装器（独享 {@link VersionedRelationRegistry} 与 pipe 配置；
    ...
    public static void main(String[] args) throws Exception {
        ...
        PipeConfig pipeConfig = PipeConfig.fromSystemProperties();
        LOG.info("pipe 配置: dir={} rollCycle={}", pipeConfig.dir(), pipeConfig.rollCycle());
