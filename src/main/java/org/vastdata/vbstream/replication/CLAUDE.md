...
- **`run(listener)`**：**阻塞消息循环，由调用方线程执行**（Main/harness 中是名为 `pgoutput-reader` 的线程：harness 里为守护线程，Main 里为普通线程、靠 shutdown hook + CountDownLatch 收敛）。**每次调用新建** `PgOutputDecoder` 与 `RelationRegistry`（Relation 缓存与 inStream 状态不跨 run 存续）；声明 `throws SQLException, IOException`（后者来自 `stream.read()`）：
  1. `stream.read()` 阻塞取下一条；**返回 null 视为连接断开**，抛描述性 `SQLException`（不包装，保留原始语义）
  2. `PgOutputDecoder.decode()` → `RelationRegistry.accept()`（ Relation 元数据入缓存）→ `listener.onMessage(msg, registry)` 回调
  3. 每条消息后 `setAppliedLSN/setFlushedLSN(getLastReceiveLSN())`；每满一个反馈周期 `forceUpdateStatus()` 上抛确认位点
- **`close()`**：顺序 流 → 复制连接 → SQL 连接；关流会令阻塞中的 `read()` 抛异常从而结束 run 循环。各步失败仅 WARN 不上抛。
