...
- 坐标：聚合 parent `org.vastdata:vb-stream-logstash:1.0-SNAPSHOT`（packaging=pom，Vastbase 生态）+ 四模块：`vb-stream-engine`（现有引擎：protocol / replication / Main / ConsoleRenderer）、`vb-stream-file-format`（VBFG 落地文件契约层——纯 JDK 零依赖，见 vb-stream-file-format/CLAUDE.md）、`vb-stream-connector-postgres-stream`（Debezium 流式连接器）与 `vb-stream-reader`（debezium-embedded 宿主冒烟应用）
