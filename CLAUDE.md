...
vb-stream-reader（embedded 宿主冒烟，前置同上 docker compose PG；详见其模块 CLAUDE.md）：
...
# 全部 Debezium/连接器配置加 vb. 前缀即透传（-Dvb.slot.streaming=parallel 等）；
...
- **源码结构**（各源码根一行；包内细节见各模块级 CLAUDE.md，层间关系见上文“架构总览”）：
    ...
    - `vb-stream-reader/src/main/java`：`org.vastdata.vbstream.reader`（debezium-embedded 宿主冒烟应用，依赖 connector 模块——`Main` 装配入口、`ReaderProperties` 的 `-Dvb.*` 剥前缀**零映射透传** + reader 默认注入（connector.class 固定 / name / offset 文件 / flush 间隔）、`LogChangeConsumer` 实现 `DebeziumEngine.ChangeConsumer`（批回调逐条 INFO 到专用 logger `org.vastdata.vbstream.reader.cdc` + markProcessed/markBatchFinished 手动 offset 记账）、`EngineLifecycle` 的 `DebeziumEngine.create(Connect.class)` 装配 + engine-run 线程 + latch/hook 收敛闸门（completed 守卫跳过已停引擎的 close、ISE 兜底重试）——Main 与 IT 共用；**slf4j-api 必须显式 2.0.17** 压 debezium 传递的 1.7.36（否则 NOP 静默无日志），坑位见模块 CLAUDE.md）
    - `vb-stream-reader/src/test/java`：`it` 包 Testcontainers IT——`ReaderPgEnv` 单例 postgres:18 容器（连接器 IT 配方翻译裁剪）+ `ReaderEndToEndIT` 两场景（①端到端：3 行 INSERT 断言数据记录 op=c/id 齐 + 事务元数据 BEGIN/END 对 + 零 op=r；②重启无重复：同槽同 offset 文件两轮运行，等 standby flush_lsn 覆盖已收记录 max lsn 再停机——确认位基线取记录自身 endLsn 而非 pg_current_wal_lsn，前沿封顶永远到不了 WAL 末端），需本机 Docker
