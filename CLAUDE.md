...
- **源码结构**（各源码根一行；包内细节见各模块级 CLAUDE.md，层间关系见上文“架构总览”）：
    ...
    - `vb-stream-file-format/src/main/java`：`org.vastdata.vbstream.format`（VBFG 落地文件契约层——文件布局/跨仓同步契约/关键不变量见模块 CLAUDE.md）
    - `vb-stream-file-format/src/test/java`：`format` 包 Varint/ChangeFileIO RoundTrip 与破坏检测单测（12 用例，零 Docker秒级）
    ...
    - `vb-stream-reader/src/main/java`：`org.vastdata.vbstream.reader`（debezium-embedded 宿主冒烟应用，依赖 connector 模块——`Main` 装配入口、`ReaderProperties` 的**三层合并**配置面（classpath `dbconfig.properties` 基础值 → `-Dvb.*` 剥前缀零映射覆盖 → reader 默认注入（connector.class 固定 / name / offset 文件 / flush 间隔）；`-Dvb.config=<路径>` 整份换外部文件，保留键不进 Debezium props）、`LogChangeConsumer` 实现 `DebeziumEngine.ChangeConsumer`（批回调逐条 INFO 到专用 logger `org.vastdata.vbstream.reader.cdc` + markProcessed/markBatchFinished 手动 offset 记账）、`EngineLifecycle` 的 `DebeziumEngine.create(Connect.class)` 装配 + engine-run 线程 + latch/hook 收敛闸门（completed 守卫跳过已停引擎的 close、ISE 兜底重试）——Main 与 IT 共用；**file 输出形态已落**（2026-09-11，`vb.sink.mode=file`：`FileChangeConsumer` 落地 VBFG 二进制文件——学习 vb-cdc-file-transform 仓 cdc-capture 的 tmp→fsync→原子 rename + COMMIT 边界切分 + offset 与 publish 严格联动链路，VBFG 契约层已抽 `vb-stream-file-format` 模块、`file` 包为解析/滚动/publish/consumer，产物与该仓 cdc-sink 消费端逐字节互通；移植偏离一处：无时区 timestamp 按 UTC 读墙钟 + 微秒保真（修正移植源的 8h 偏差与毫秒截断）；细节见模块 CLAUDE.md）；**slf4j-api 必须显式 2.0.17** 压 debezium 传递的 1.7.36（否则 NOP 静默无日志），坑位见模块 CLAUDE.md）
    - `vb-stream-reader/src/test/java`：`ReaderPropertiesTest` 离线单测（三层合并次序/classpath 模板/外部文件替换与 fail-fast/必填校验与打码/sink 键合并与 Debezium 剥离）+ `file` 包 `FileChangeConsumerTest`/`FileRollingWriterTest`（offset 联动/时间切分/seq 恢复）+ `it` 包 Testcontainers IT——`ReaderPgEnv` 单例 postgres:18 容器（连接器 IT 配方翻译裁剪）+ `ReaderEndToEndIT` 两场景（①端到端：3 行 INSERT 断言数据记录 op=c/id 齐 + 事务元数据 BEGIN/END 对 + 零 op=r；②重启无重复：同槽同 offset 文件两轮运行，等 standby flush_lsn 覆盖已收记录 max lsn 再停机——确认位基线取记录自身 endLsn 而非 pg_current_wal_lsn，前沿封顶永远到不了 WAL 末端）+ `ReaderFileSinkIT`（file 形态端到端：六类型列落地 VBFG 文件 → 移植 Reader 读回断言记录序/表定义/值墙钟原值往返/txid 同源/CRC 完整），需本机 Docker
