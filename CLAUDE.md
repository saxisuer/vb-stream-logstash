...
- 坐标：聚合 parent `org.vastdata:vb-stream-logstash:1.0-SNAPSHOT`（packaging=pom）+ 六模块：`vb-stream-engine`（现有引擎：protocol / replication / Main / ConsoleRenderer）、`vb-stream-file-format`（格式层公共基座——文件命名契约 + 事件 IR，纯 JDK 零依赖，见其 CLAUDE.md）、`vb-stream-binary-format`（VBFG 二进制落地格式——记录模型 + 二进制流式读写，依赖基座）、`vb-stream-sql-format`（SQL 文本落地格式——IR → 可执行语句渲染（PG/VastBase 方言），依赖基座）、`vb-stream-connector-postgres-stream`（Debezium 流式连接器，设计见 docs/superpowers/specs/2026-09-01-debezium-connector-postgres-stream-design.md）与 `vb-stream-reader`（debezium-embedded 宿主冒烟应用：`DebeziumEngine.create(Connect.class)` 加载连接器，`ChangeConsumer` 批回调逐条 SourceRecord 输出 slf4j 日志，`-Dvb.*` 剥前缀透传配置，见其 CLAUDE.md）
- 工具链：Java 17 + Maven
- 当前状态：**里程碑 2.0 已完成**（在 pgoutput 流式解码器、复制会话（raw 字节接缝）、事务组装与读取/输出解耦之上，**输出契约流式化**：`StreamingTransactionListener.onEvent(TransactionEvent)` 单回调事件交付（`Begin → TxChange* → End`），回放期逐条解码逐条交付、堆峰从 O(事务) 降到 O(单条)；`End` 返回 = 下游确认完整消费，输出前沿随 End 推进；block 逃生门 `vb.output.mode=block` 经 `StreamingToBlockAdapter` 在输出边界攒回整块恢复 1.7 原子交付语义；输出格式（TXN-BEGIN/逐行/TXN-END）逐字节不变，引擎 `mvn test` 224 用例全绿；**吞吐与分布指标已内建**（`ThroughputMetrics`——slot 读取/组装/输出三段速率 + 回放耗时/事务大小分位数 + 八项会话峰值行，10s 周期 INFO 日志行，2026-08-31 设计）；JMH 基线含 2.0 契约换血对照段（`docs/benchmarks-baseline.md`，另含 2026-08-31 端到端在线吞吐基线段：单 slot 双上限——窄行 ~34 万条/s、宽行 ~320MB/s，组装 ~4.4 万 tx/s，Main 全程非瓶颈）；**1.7/1.7.1 成果沿用**——读取与组装输出解耦（reader 记账 + `MessagePipe` 主缓冲 + consumer 回放、LSN 确认按输出前沿封顶、at-least-once）；组装成本归因三腿互证（存储路径 ≈58%、缺页 ≈46.8% 属固有；deletableFiles 节流 −38.3%；1.7.2 判定不开混合存储，见 baseline 文档 1.7.1 段））；**pgoutput binary 值模式已落**（2026-09-09：`vb.pg.binary=true` 开 START_REPLICATION 的 binary 选项（PG 16+，仅 true 时传参），数据列以 typsend 二进制走 `'b'` 种类——wire 解码 `'b'` 自 PG 16 支持期已内建；`BinaryValueDecoder` 按列 typeId 把 'b' 载荷解为与 text 模式对齐的可读值（bool/整型/浮点/文本/bytea/numeric 无损/date/time/timetz/timestamp(tz)/uuid/json 标量矩阵 + interval（interval_out postgres 风格，负值分解）+ 18 种内建数组（array_send 元素**统一 i32 长度前缀、NULL=-1**——定长与 varlena 同构，bool 的 false 与 NULL 由此无歧义，PG 18 实测锚定；array_out 引号/反斜杠转义/非零 lowerBound 前缀）），`ConsoleRenderer` 渲染接线、矩阵外类型（jsonb/enum/域/组合类型/自定义数组）降级十六进制 + WARN 一次，端到端归 `BinaryOutputTest` 四场景含五类型族（时间/数字/字符串/interval/数组）专项）。核心依赖（版本以 pom 的 `<properties>` 为准）：
...
- **superpowers 技能已放开使用**（2026-09-11 起，此前曾禁用）：创建功能/组件/修改行为前走 `superpowers:brainstorming`，实现走 `superpowers:test-driven-development`，排障走 `superpowers:systematic-debugging`，多步任务先 `superpowers:writing-plans`——按技能指引的流程执行；本节其余规约（方法名英文 camelCase、import 简名、slf4j、javadoc 等）对技能执行过程同样适用
...
#           -Dvb.pg.binary=true（pgoutput 二进制值模式，PG 16+，2026-09-09 落地——'b' 载荷按列
#           typeId 经 BinaryValueDecoder 解码为与 text 模式对齐的可读值；标量全矩阵 + interval +
#           18 种内建数组（元素统一 i32 长度前缀、NULL=-1）；jsonb/enum/域/组合类型等矩阵外降级
#           十六进制 + WARN 一次，见 protocol/CLAUDE.md）
#           -Dvb.pipe.dir=... -Dvb.pipe.rollCycle=... -Dvb.output.mode=streaming|block
```

vb-stream-reader（embedded 宿主冒烟，前置同上 docker compose PG；详见其模块 CLAUDE.md）——配置默认读 classpath 的 `dbconfig.properties`（src/docker 模板，**零 -D 参数即可起**）：
...
# 配置三层合并: dbconfig.properties(基础值) → -Dvb.<键>=<值>(覆盖,如 -Dvb.slot.streaming=parallel)
#              → 默认兜底;整体换文件 -Dvb.config=<绝对路径>(免重编译)
# 输出形态切 file 落地: -Dvb.reader.mode=file(可加 -Dvb.reader.format=sql 选 SQL 文本格式,
#              默认 binary=VBFG 二进制;目录/滚动参数见 reader 模块 CLAUDE.md)
# Windows classpath 分隔符 ';'，macOS/Linux 为 ':'；CDC 记录行走专用 logger
# org.vastdata.vbstream.reader.cdc；引擎 offset 默认落 data/reader-offsets.dat（已 gitignore）
```

- **运行期每 10s 打四行统计 INFO**（consumer 统计行 + `吞吐:` 行 + `分布:` 行 + `峰值:` 行——`ThroughputMetrics`，指标常开无配置面；吞吐按窗口差分（观察面）、分位按区间隔离、峰值速率为**会话最高单秒速率**（秒桶——窗口均值会把突发摊薄 ~窗口/突发时长之倍数，峰值行须反映真实突发），口径与格式见 `replication/CLAUDE.md` 的 ThroughputMetrics 节）
...
- **源码结构**（各源码根一行；包内细节见各模块级 CLAUDE.md，层间关系见上文“架构总览”）：
    ...
    - `vb-stream-engine/src/test/java`：`protocol`/`replication` 包字节级单测（`MsgBuilder`/`PgWire` 手造字节辅助）与顶层 `ConsoleRendererTest`、`it` 包集成测试 14 组（Testcontainers，见其 CLAUDE.md）、`bench` 包语料基建（JMH 语料来源）
    - `vb-stream-engine/src/jmh/java`：五基准（`-Pjmh` 档才参与编译，默认构建零 JMH 依赖，见其 CLAUDE.md）
    - `vb-stream-file-format/src/main/java`：`org.vastdata.vbstream.format`（格式层公共基座——`FileNaming` 文件命名契约（bin/json/sql 三后缀）+ 事件 IR（`TableDef`/`ColumnDef`/`Op`/`TypeCode`/`ParsedEvent`），binary/sql 两格式模块依赖本基座；枚举持久化警告与不变量见模块 CLAUDE.md）
    - `vb-stream-file-format/src/test/java`：`format` 包 `FileNamingTest`（3 用例——三后缀命名组装/seq 解析/nextSeq 恢复，零 Docker 秒级）
    - `vb-stream-binary-format/src/main/java`：`org.vastdata.vbstream.format.binary`（VBFG 二进制落地格式——`Record` 记录家族六类 + `ChangeFileWriter`/`ChangeFileReader` 流式读写 + `io` 子包 `Varint`/`Values`；二进制布局/跨仓同步契约/关键不变量（含不识别对方仓 DDL(7) 记录的已知限制）见模块 CLAUDE.md）
    - `vb-stream-binary-format/src/test/java`：`binary` 包 `VarintTest`/`ChangeFileIOTest`（12 用例，2026-09-16 自基座随模块迁移、断言零改动，零 Docker 秒级）
    - `vb-stream-sql-format/src/main/java`：`org.vastdata.vbstream.format.sql`（SQL 文本落地格式——`SqlRenderer` IR → 可执行语句（PG/VastBase 方言），纯函数零状态，与对方仓分叉点唯一：无 renderDdl；渲染规则表/两处 VastBase 实测裁定/已知限制见模块 CLAUDE.md）
    - `vb-stream-sql-format/src/test/java`：`sql` 包 `SqlRendererTest`（10 用例，零 Docker 秒级）
    - `vb-stream-connector-postgres-stream/src/main/java`：`org.vastdata.debezium.connector.postgresql.stream`（Debezium 流式连接器；MS1 已落协议层 + 三件套骨架——`protocol` 子包 pgoutput 19 消息纯函数解码 + 双入口 `PgOutputStreamDecoder`（引擎 protocol 包 1:1 重写、零引擎 import，见其 CLAUDE.md）；**MS2 已落端到端**——自建复制会话（`ReplicationSession`：raw drain + LSN 反馈按输出前沿封顶 + 幂等建槽带 two_phase）、CQ 管道（`MessagePipe`）与异步桶记账组装（`StreamedTransactionAssembler`/`VersionedRelationRegistry`/`RelationTableFactory` 的 'R' JDBC enrich）、双线程（vb-pgoutput-reader/transaction-consumer）+ 监督壳（`PostgresStreamStreamingChangeEventSource`：心跳/停机 D7 shutdownFast 不排干）、回放发射（`BucketReplayer`/`DispatcherTransactionListener`→Debezium dispatcher，事务边界 offset、事务 id 纯数字与元数据提供者同源）与 `PostgresStreamConnectorTask` 真装配（含 vanilla 命名豆注册、initialContext 读后 commit——防 CREATE SLOT 自死锁）；R1/R3 线程审计结论在档 `docs/superpowers/specs/2026-09-02-ms2-r1-r3-audit.md`；**MS3 已落语义闭环**——Truncate 发射（`TruncateEmitter`，`skipped.operations` 门控照 vanilla：默认 "t" 跳过、`none` 才逐表发 key=null/op="t" 记录）+ 三情况端到端 IT（aborted 子事务剔除/同事务 DDL asOf 前后段分界/重启三情况——半事务停机 D7 重发补齐、offset 落后重复取并集、无缝续传）+ 数组列 fail-fast 等已知限制与 LogicalMsg 延期记档（同上审计文档「已知限制与延期」节）；**MS3.5 已落 'M' 逻辑消息记录与安全推进**——`slot.messages` 门控（默认 false，false 时槽选项与行为同 MS3）下解析记录不发射（INFO 两时点：非事务 reader 即时/事务性 consumer 回放期）+ 非事务消息经护栏即时推进前沿（全有或全无：无未输出桶才推进到消息位，有则完全静止；crash 注入 IT 实证未输出事务重启整桶重发、尾部不丢；发射仍延期，见审计文档第 3 条转「部分实现」）；配置面新增 `slot.streaming`/`slot.two.phase`/`pipe.dir`/`pipe.roll.cycle`/`slot.feedback.interval.ms`/`slot.messages` 六项，parallel 档未开 two_phase 启动期校验 fail-fast；**全串输出开关 `values.as.string`（默认 false）**——true 时值+schema 双轨改形：Task.start 的 schema 转换器换 `StringValueConverter`（所有列含 key 恒 STRING + converter 恒等）、监督壳 execute 的值映射器换 `StringColumnValueMapper`（pgoutput 文本原文透传，TOAST 占位沿用 vanilla `unavailable.value.placeholder`），两处必须同开关切换否则 Struct 层 DataException；数组列 fail-fast 与未知类型静默 null 两项已知限制在此模式均消解（数组原文透传不碰 PgArray 活连接），numeric 精度无损、时间无 epoch 时区换算，代价是与 Debezium 标准事件形态分叉（下游全 VARCHAR、布尔 t/f、bytea \\x 文本），端到端归 `StringValuesIT`；**MS4 已落 two_phase 语义闭环**——两阶段 IT 四场景（挂起期零发射/弃桶/parallel 档 StreamPrepare 大事务/挂起期停机重启续传）+ R5 存量槽预检（42710 复用前查 `pg_replication_slots.two_phase`，不匹配启动期拒绝附 DROP SLOT 迁移指引）；**MS5 已落集成面收官**——配置面钉死 snapshot.mode 仅 no_data（同名替换父 Field、默认注入，REST validate 真实生效 + 任务构造器 fail-fast 兜底）与 provide.transaction.metadata 默认 true（taskConfigs 注入）+ 管线指标同构复刻（`StreamThroughputMetrics` 五点插桩：reader 记 slot 读取、组装记交接、consumer 记输出/分布/峰值、回放 sink 逐条 rec 秒桶——第五点 2026-09-07 修正：整事务 End 落桶会虚高单大事务的输出峰值，与引擎同日同构，10s INFO 三行与引擎同口径）+ MBean 面（`StreamStreamingChangeEventSourceMetrics` 经 Debezium metrics 体系暴露五速率/lagBytes/挂起 prepared 数/管道磁盘占用——`StreamMetricsBridge` 挂统计 tick 预计算，JMX 读零锁）+ R2 增量快照交错审计只审不接（结论：建议 MS6 接 signal-based 形态、不接 read-only 变体，前置条件五项在档 `docs/superpowers/specs/2026-09-05-ms5-r2-incremental-snapshot-audit.md`）；**MS6 已落打包与文档收官**——maven-assembly plugin 目录清单打包（`src/main/assembly/plugin.xml`：连接器自身 jar 落插件根 + runtime 依赖落 `lib/`，connect-api/kafka-clients/slf4j-api 及其独占子件四件显式排除——R4 两连接器并存清单边界，产物 `target/vb-stream-connector-postgres-stream-plugin/` 目录 + 同名 zip）、`ConnectPluginIT` 真 Kafka Connect 验收（cp-kafka-connect 8.3.0 容器 plugin.path 挂插件 → REST PUT 扁平 config map 建连接器 → PG 写入 → topic 收数断言）、R2 裁定记档（v1 不接 signal-based 增量快照，审计文档结论节裁定行）与模块 README（定位/配置面/打包安装/at-least-once 语义/已知限制五节一档全，配置表逐项对照 `PostgresStreamConnectorConfig` javadoc 防漂移），见其 CLAUDE.md）
    - `vb-stream-connector-postgres-stream/src/test/java`：`protocol` 包字节级单测（`MsgBuilder` 手造字节）+ 三件套/组装/回放/接缝单测（含全串开关的 `StringColumnValueMapperTest`/`StringValueConverterTest`），**272 用例含 `it` 包 27 用例**——embedded engine（`AbstractAsyncEngineConnectorTest` 基座）+ Testcontainers 真 PG 的端到端 IT（`StreamPgTestEnv`/`StreamITBase` + `EndToEndStreamedTxIT` 流式大事务进 Kafka 记录/重启续传/亚秒反馈、`StringValuesIT` 全串模式双场景（values.as.string=true 六类型列值+schema 全 STRING 含数组原文流动/缺省时类型化基准不受扰）、`TruncateIT` 双表 TRUNCATE 逐表发射/默认门控零记录、`StreamAbortFilterIT` SAVEPOINT 子事务回滚剔除端到端（被回滚行不进 Kafka/存活行完整/END 计数=实付）、`InTxnDdlAsOfIT` 同事务 DDL 前后段各按变更时刻表结构渲染（列数分界正确）、`RestartSemanticsIT` 重启三情况（半事务停机 D7 重发补齐/offset 落后重复取并集/无缝续传）、`LogicalMsgIT` 逻辑消息三场景（纯消息流不钉死 confirmed_flush/crash 注入护栏钉住未输出事务重启尾部不丢/中段停机整事务重发取并集）、`TwoPhaseIT` 两阶段四场景（PREPARE 挂起零发射/ROLLBACK PREPARED 弃桶/parallel 档 StreamPrepare 大事务全量落 Kafka/prepared 挂起期停机重启续传）、`SlotTwoPhaseMismatchIT` 存量槽 two_phase 不匹配启动期拒绝（失败信号经基座 CompletionCallback 捕获、异常链含 DROP SLOT 指引、槽不删）、`DefaultsAndMetricsIT` 集成面三场景（缺省配置注入 no_data+事务元数据/snapshot.mode=initial 启动期拒绝/10s 统计 tick INFO 行可观测）、`ConnectPluginIT` 真连接器验收（不挂基座：cp-kafka 8.3.0 + cp-kafka-connect 8.3.0 容器组 plugin.path 挂 assembly 插件目录 → REST `PUT /connectors/{name}/config` 扁平 config map 建连接器 → INSERT → topic 收数断言 op 恰 {c}/零 op=r + 事务元数据 BEGIN/END；@BeforeAll 产物结构断言前置，缺产物 fail-fast 提示先 `mvn -pl vb-stream-connector-postgres-stream package -DskipTests`）、`ReaderUnblockedIT`/`FrontierCapIT`/`ReaderThroughputIT` 解耦三件翻译，需本机 Docker；基座消费断言在 JDK 17 链接期引 Confluent 类故用免校验替身，坑位见模块 CLAUDE.md）
    - `vb-stream-reader/src/main/java`：`org.vastdata.vbstream.reader`（debezium-embedded 宿主冒烟应用，依赖 connector 模块——`Main` 装配入口、`ReaderProperties` 的**三层合并**配置面（classpath `dbconfig.properties` 基础值 → `-Dvb.*` 剥前缀零映射覆盖 → reader 默认注入（connector.class 固定 / name / offset 文件 / flush 间隔）；`-Dvb.config=<路径>` 整份换外部文件，保留键不进 Debezium props）、`LogChangeConsumer` 实现 `DebeziumEngine.ChangeConsumer`（批回调逐条 INFO 到专用 logger `org.vastdata.vbstream.reader.cdc` + markProcessed/markBatchFinished 手动 offset 记账）、`EngineLifecycle` 的 `DebeziumEngine.create(Connect.class)` 装配 + engine-run 线程 + latch/hook 收敛闸门（completed 守卫跳过已停引擎的 close、ISE 兜底重试）——Main 与 IT 共用；**file 输出形态已落，binary/sql 双格式**（2026-09-11 落地，2026-09-16 拆格式层并增 sql 格式，`vb.reader.mode=file`：`FileChangeConsumer` 落地为文件——学习 vb-cdc-file-transform 仓 cdc-capture 的 data 目录内 .part 同目录暂存→fsync→原子 rename 去后缀 + COMMIT 边界切分 + offset 与 publish 严格联动链路（2026-09-16 同步对齐对方仓 tmp 目录合并进 data 目录的 refactor——单 dataDir，tmp-dir 键废弃 WARN 忽略），`vb.reader.format=binary|sql`（默认 binary）经 `OutputFormat` 枚举工厂选格式——binary 走 VBFG 二进制（产物与该仓 cdc-sink 消费端逐字节互通），sql 走 `SqlRenderer` 渲染可执行语句文本；格式层已拆三模块（binary-format/sql-format 依赖基座 file-format），`file` 包为解析/滚动/publish/consumer 的格式无关外壳；移植偏离一处：无时区 timestamp 按 UTC 读墙钟 + 微秒保真（修正移植源的 8h 偏差与毫秒截断）；细节见模块 CLAUDE.md）；**slf4j-api 必须显式 2.0.17** 压 debezium 传递的 1.7.36（否则 NOP 静默无日志），坑位见模块 CLAUDE.md）
    - `vb-stream-reader/src/test/java`：`ReaderPropertiesTest` 离线单测（三层合并次序/classpath 模板/外部文件替换与 fail-fast/必填校验与打码/`reader.*` 键合并与 Debezium 剥离/输出形态默认值与 `reader.format` 非法值 fail-fast）+ `file` 包 `FileChangeConsumerTest`（offset 联动）/`FileRollingWriterTest`（时间切分/seq 恢复/.part 残留清理[只删半成品,非 .part 不动] + sql 格式扩展——后缀 publish 与文件内容形态）/`SqlEventWriterTest`（头注释/BEGIN/语句/COMMIT 逐行文本面 + 中文载荷 UTF-8 往返防回归）+ `it` 包 Testcontainers IT——`ReaderPgEnv` 单例 postgres:18 容器（连接器 IT 配方翻译裁剪）+ `ReaderEndToEndIT` 两场景（①端到端：3 行 INSERT 断言数据记录 op=c/id 齐 + 事务元数据 BEGIN/END 对 + 零 op=r；②重启无重复：同槽同 offset 文件两轮运行，等 standby flush_lsn 覆盖已收记录 max lsn 再停机——确认位基线取记录自身 endLsn 而非 pg_current_wal_lsn，前沿封顶永远到不了 WAL 末端）+ `ReaderFileOutputIT`（file 形态端到端双格式：binary——六类型列落地 VBFG 文件 → 移植 Reader 读回断言记录序/表定义/值墙钟原值往返/txid 同源/CRC 完整；sql——落地 .sql 在同容器镜像表执行回查断言六类型列值往返），需本机 Docker
- 集成测试（`org.vastdata.vbstream.it`，14 组）经 Testcontainers 自动起容器（postgres:18 单例 `PgTestEnv`；源库降级 PG 17 的兼容实证另起 postgres:17 单例 `Pg17TestEnv`，仅 `Pg17CompatTest` 引用——Relation typmod 对齐 / binary 五类型族 / 流式 binary 大事务 / two_phase / StreamAbort parallel 附加字段五场景，2026-09-11 REL_17/18_STABLE 源码审计配套），需本机 Docker；`mvn test` 单命令跑全部。解耦专项三组：`DecoupledPipelineTest` 三场景（①双连接并发流式大事务多桶交错 + StreamAbort 子事务剔除，异步管道双回放输出全等 ②大事务内同事务 DDL，前后段按 asOf 版本渲染 ③流式大事务回滚后低水位推进 + 陈旧滚动文件实际删档）、`ReaderUnblockedTest`（头名验收——consumer 阻塞期间 reader 持续接收，放行后排干不丢不重）、`FrontierCapTest`（未输出事务钉住 confirmed_flush，输出后越过封顶）；另有 `ReaderThroughputTest` 读取节拍回归（500 行事务 35s 内录完——防"每轮一条+固定 sleep 100ms"的 ~10 msg/s 节拍退化，2026-08-31 吞吐冒烟实测踩坑）；`BenchCorpusRecordTest` 为基准语料生成器（语料缺失或场景脚本 SHA-256 指纹变化才起容器重录，指纹一致时秒过）
