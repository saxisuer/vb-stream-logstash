...
- **源码结构**（各源码根一行；包内细节见各模块级 CLAUDE.md，层间关系见上文“架构总览”）：
    ...
    - `vb-stream-engine/src/test/java`：`protocol`/`replication` 包字节级单测（`MsgBuilder`/`PgWire` 手造字节辅助）与顶层 `ConsoleRendererTest`、`it` 包集成测试 13 组（Testcontainers，见其 CLAUDE.md）、`bench` 包语料基建（JMH 语料来源）
