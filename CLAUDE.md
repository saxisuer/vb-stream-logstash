...
- **源码结构**（各源码根一行；包内细节见各模块级 CLAUDE.md，层间关系见上文“架构总览”）：
    ...
    - `vb-stream-connector-postgres-stream/src/test/java`：`protocol` 包字节级单测（`MsgBuilder` 手造字节）+ 三件套/组装/回放/接缝单测，**194 用例含 `it` 包 9 用例**——embedded engine（`AbstractAsyncEngineConnectorTest` 基座）+ Testcontainers 真 PG 的端到端 IT（`StreamPgTestEnv`/`StreamITBase` + `EndToEndStreamedTxIT` 流式大事务进 Kafka 记录/重启续传/亚秒反馈、`TruncateIT` 双表 TRUNCATE 逐表发射/默认门控零记录、`ReaderUnblockedIT`/`FrontierCapIT`/`ReaderThroughputIT` 解耦三件翻译，需本机 Docker；基座消费断言在 JDK 17 链接期引 Confluent 类故用免校验替身，坑位见模块 CLAUDE.md）
