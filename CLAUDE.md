...
- **源码结构**（各源码根一行；包内细节见各模块级 CLAUDE.md，层间关系见上文“架构总览”）：
    ...
    - `vb-stream-connector-postgres-stream/src/main/java`：`org.vastdata.debezium.connector.postgresql.stream`（Debezium 流式连接器；MS1 已落协议层 + 三件套骨架——`protocol` 子包 pgoutput 19 消息纯函数解码 + 双入口 `PgOutputStreamDecoder`（引擎 protocol 包 1:1 重写、零引擎 import，见其 CLAUDE.md），父包 Connector/Config/Task 三件套最小骨架（不连库、poll 恒空）；配置面新增 `slot.streaming`/`slot.two.phase`/`pipe.dir`/`pipe.roll.cycle` 四项，parallel 档未开 two_phase 启动期校验 fail-fast）
    - `vb-stream-connector-postgres-stream/src/test/java`：`protocol` 包字节级单测（`MsgBuilder` 手造字节）+ 三件套/日志绑定单测，56 用例（不起 PG、不连库）
