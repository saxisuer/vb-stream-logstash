...
重记（复测材料 `~/perf/run/` 仍在；**已于 2026-09-08 完成，见文末「WSL 修复后复测」段**）；
connector 段全部数字不受影响。判读通则：**输出 rec/s
...
### WSL 修复后复测（2026-09-08，metrics 双修复后三档全量——connector 形态）

**性质**：metrics 双修复（Debezium 热路径覆写 + rec 秒桶逐条化，b13a2b4）后的 WSL 三档复测——
补齐上段缺的 WSL 修复后 512B/64KB 精确值，slot 供给域与输出链同轮对表。**环境与 2026-09-07
WSL 段同一台 VM**（PG 18.6 / `logical_decoding_work_mem=512MB` / loopback / cdc logger OFF），
connector jar 重装至修复后构建（本地仓库 175KB 那份）；复测材料仍 `~/perf/run/`（start-reader.sh
+ 外部 logback + full-cp.txt）。**三档各起独立 reader 会话**——峰值行是会话累计 max，混跑会
互相吞峰值（如 8B 轮的 rec 秒峰与 512B 轮不可比大小）；每会话删 offsets 重建档槽（位点从
TRUNCATE 前起，负载 = TRUNCATE(被 skipped.operations 滤零) + 单事务 INSERT）。

| 档（单事务） | slot 峰值 | 2026-09-07 基线 | 输出秒峰（修复后口径） | 回放全程（时序差） | 输出均值 |
|---|---|---|---|---|---|
| 8B×200 万 | 11.9 MB/s（**361,740 msg/s**） | 374,097（−3%） | **369,608 rec/s** / 12.2 MB/s | 6.5s | **30.8 万 rec/s** / 11.1 MB/s |
| 512B×100 万 | **129.3 MB/s**（239,395 msg/s） | 133.0（−3%） | **329,118 rec/s** / 177.7 MB/s | 4.0s | **25.0 万 rec/s** / 135 MB/s |
| 64KB×1.5 万 | **365.3 MB/s**（5,572 msg/s） | 419.4（−13%） | 8,297 rec/s / **544 MB/s** | 2.1s | 7.1k rec/s / **469 MB/s** |

**结论**：

- **slot 供给域同量级复现**：三档 −3%~−13%（VM 状态噪音带），条数域 36 万 / 字节域 365 MB/s
  的"walsender 单流双上限"结论不变。
- **输出链修复后与供给域同量级**：8B 档输出秒峰 369,608 rec/s ≈ slot 峰 361,740 msg/s；回放
  6.5s/30.8 万与 2026-09-07 Arthas 修复段**精确同值复现**；512B 档均值 25.0 万 rec/s 为 WSL
  修复后首测（修复前 3.77 万，**6.6 倍**）。"Debezium 化全链上限 = walsender 供给 × 行宽"
  的修正结论坐实。
- **输出秒峰可高于 slot 秒峰属形态自洽**（8B 36.9 万>36.2 万、64KB 544>365 MB/s）：单事务
  形态下读取与回放**时间线不重叠**（reader 全收完 Commit 交接后 consumer 才开始），两侧秒峰
  各取各的最快秒；回放均值高于 slot 峰值即"consumer 排干管道快于 walsender 发送"（CQ 顺序
  读 vs 网络+协议栈），非记账异常。
- **HDR 量化注记**：分布行"事务大小"读回 2,007,039/1,003,519/15,039（记录 200 万/100 万/
  1.5 万，+0.35% 偏差）——2 位有效数字的桶级量化（本文档已知口径限制节的既注行为），均值
  复算用真值行数而非量化读回值。
- **macOS 段对照升级**：512B 修复后 WSL 25.0 万 vs macOS 5.6 万 rec/s——4.5×，与 slot 供给
  域的环境差同向叠加，"判读先看环境"通则再证。
