...
- **切换 = 纯字节转储**：内存桶越限 → 已有单元逐条加信封帧（seq + 可选 streamXid，§5）后 `writeBytes` 进 CQ、后续单元直写 CQ。自始至终只有一种存储格式（pgoutput 原始字节 + 定长信封帧头），不存在"数据转移时的表示形态变化"
...
| 组件 | 角色 |
|---|---|
| `MessageSpool`（新） | CQ 单例管理者：`append(bytes)→index`、`dump(List<PayloadUnit>)→(first,last)`、`readRange(first,last)` 回读、全局单调序号分配、低水位删滚动文件、close 顺序收敛。持有 ChronicleQueue/Appender/Tailer 生命周期 |
...
| `TransactionAssembler` 改造 | **payload 路由窥探内置于组装器**（非独立组件）：只读类型字节 + 流块内 Int32 xid 前缀即可选桶，不完整解码；inStream 上下文由 live 解码的 StreamStart/StreamStop 驱动，不需独立状态机。桶改持 `List<PayloadUnit>`（MEMORY）或 `(firstIndex,lastIndex)`（SPILLED）+ 字节记账；新增全局水位与 spillAll 触发；提交路径改为"遍历单元 → 滤 abortedSubxids → `decodeSingle` → 组 TxChange → 封箱"（回放不路由） |

内存侧每桶只剩：xid、gid（2PC）、kind、mode、`List<PayloadUnit>`（MEMORY）或 index 区间（SPILLED）、abortedSubxids 集合、字节计数——几十字节到 KB 级；SPILLED 后逐单元元数据只存在于 CQ 信封帧，堆内为零。100GB 事务的堆占用也是平的。
...
```
关桶 → 回放：MEMORY 遍历 List<PayloadUnit> / SPILLED tailer 读 [first..last] 逐条解信封帧
...
- **SPILLED 单元带信封帧**：入队字节 = 信封帧（seq + 可选 streamXid + payload，`SpoolFrame` 纯函数帧/解帧），堆内不保留逐单元元数据——桶只记 `(firstIndex, lastIndex)` 区间，seq/streamXid 随字节落盘、回读复原
