...
/**
 * 事务输出事件族:单回调流式交付。引擎 {@code org.vastdata.vbstream.replication.TransactionEvent}
 * 的 1:1 重写(文字参照,非依赖),是 connector 内部契约——不导出引擎语义。
 *
 * <p>职责:定义事务回放期对外交付的三种事件形态——事务头({@link Begin})、逐条变更
 * ({@link TxChange})、事务尾({@link End})——回放器(Task 5 的 {@code BucketReplayer})
 * 逐条解码逐条交付,不把整事务的变更累积成 List 后封箱回调,回放期堆峰值从 O(事务大小)
 * 降到 O(单条)。sealed permits 在编译期钉死成员集,下游面向一种事件类型编程
 * (Task 7 的 {@code DispatcherTransactionListener} 是事件 → Debezium dispatcher 的映射点)。
 *
 * <p>契约注记:
 * <ul>
 ...
 * consumer 线程(异步形态,Task 6;同步测试形态即调用线程)。
 */
public sealed interface TransactionEvent permits TransactionEvent.Begin, TransactionEvent.End, TxChange {
     ...
     * @param endLsn          提交结束 LSN(输出前沿按此推进,offset 事务边界同源)
     * @param commitTimestamp 提交时间戳
     * @param expectedChanges 预期变更条数(aborted 过滤<b>前</b>,reader 桶记账 unitCount);
     *                        实际交付数可能小于此值,见 {@link End}
     */
