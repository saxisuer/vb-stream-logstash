...
/**
 * 管道目录的 Windows 句柄兜底清理（测试侧共享工具；起点是 {@code ThroughputMetricsWiringTest}
 * 与 it 包 {@code DecoupledPipelineTest} 的同款私有实现，2026-09-08 提炼共用）。存在动机：
 * Chronicle Queue 的 mmap 句柄在 {@code close()} 后由 GC/cleaner **异步**释放，Windows 不允许
 * 删除仍被映射占用的文件（POSIX 的 unlink 语义无此约束——macOS/Linux/WSL 天然免疫），紧随
 * close 的两种目录操作都会撞句柄：① 类级共享 {@code @TempDir} 的下一用例构造管道
 * （{@code MessagePipe} 的 wipe-on-open 删上一用例的队列文件失败）；② {@code @TempDir} 默认
 * ALWAYS 清理的用例收尾删除。测试因此在 Windows 上系统性判红（2026-09-08 本机实测：两侧
 * 组装器大类 38/41、40/50 用例翻车），macOS/WSL 全绿从未暴露。
 *
