...
/**
 * 测试用 pgoutput 线格式字节构造器（raw 驱动组装器单测的底座）：以静态方法构造**完整单条**
 * 消息的 {@code byte[]}（含首个类型字节与其后全部字段，布局按 decoder 设计文档附录 A，
 * 与生产解析器 {@code NormalParsers}/{@code DmlParsers}/{@code StreamParsers}/{@code TwoPhaseParsers}
 * 逐一镜像）。全部整数字段 big-endian；字符串为 null 结尾 UTF-8（CString）。
 *
 * <p>占位约定（组装器单测只断言路由与组装语义，不断言服务端真实 LSN/时间戳）：
 * <ul>
 *   <li>LSN 字段：每条消息内按出现顺序填 1、2、3……递增</li>
 *   <li>微秒时间戳字段：一律填 0——decoder 换算后为 {@link #PG_EPOCH}
...
public final class PgWire {
    ...
    /** 微秒占位 0 经 decoder 换算得到的时间戳（PG 纪元 2000-01-01T00:00:00Z），断言侧引用。 */
    ...
    /**
     * 责任：构造 StreamAbort('A') 的**非 parallel 形态**——I32 顶层 xid + I32 被回滚的
     * （子）事务 xid，无附加字段。
     ...
     * （占位 1）+ I64 rollbackEndLsn（占位 2）+ **两个** I64 微秒时间戳（均占位 0）+
    ...
    /**
     * 责任：把任一已构造的顶层消息（I/U/D/T/M/R 等）包装为**流式块内形态**——
