...
/**
 * 主缓冲管道（1.7 设计 §4.2）：一条 CQ 记录 = 一条完整 pgoutput 消息（含控制消息，为建立
 * seq 时间线），无信封帧——CQ 自身的 index 即消息 seq（readRange 逐条回调真实 index），
 ...
 *
 * <p><b>线程约束</b>：跨线程分工——{@code append}/{@code lastAppendedIndex}/{@code releaseBelow}/
 * {@code close} 由 reader 线程调用，{@code readRange} 由 consumer 线程调用。appender 与
 * tailer 都是构造时创建的单实例资源，各自单线程使用；跨线程的可见性与顺序由 Chronicle Queue
 * 的单 appender/多 tailer 内存模型保证（官方支持的使用方式）。两类方法不得交叉线程调用
 * （appender 非线程安全，tailer 的 moveToIndex 游标也只属于一个线程）。
 */
